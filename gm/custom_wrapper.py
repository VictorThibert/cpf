...
class GooglePlaces(object):
    ...
    def _generate_lat_lng_string(self, lat_lng, location):
        if location is None and lat_lng is None:
            return "next-page-token?" # this occurs in between page searches
        if lat_lng is None and location is not None:
            lat_lng = {'lat':location.split(',')[0],'lng':location.split(',')[1]}
        try:
            return '%(lat)s,%(lng)s' % (lat_lng) 
